...
const data: NavItem[] = [
  ...
  { id: "triggers", label: "Triggers", icon: "▶", type: "triggers" },
...
const TRIGGER_LABELS: string[] = [
  "Mail NFE confirmation",
  "Mail T24 finish confirmation",
  "Mail settlement confirmation",
  "Mail reconciliation report",
  "Mail EOD summary",
  "Mail batch status alert",
];
...
function TriggersPanel(): JSX.Element {
  const [states, setStates] = useState<Record<string, TriggerState>>(
    Object.fromEntries(TRIGGER_LABELS.map((l) => [l, { status: "idle", message: "" }]))
  );

  const handleTrigger = async (label: string): Promise<void> => {
    setStates((prev) => ({
      ...prev,
      [label]: { status: "loading", message: "" },
    }));

    try {
      const res = await fetch(`${FASTAPI_BASE}/trigger`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        setStates((prev) => ({
          ...prev,
          [label]: { status: "error", message: `Error ${res.status}: ${text}` },
        }));
      } else {
        ...
        setStates((prev) => ({
          ...
          [label]: {
            status: "success",
            message: json?.message ?? "Triggered successfully",
          },
        }));
      }
    } catch (err) {
        ...
        <div className="flex flex-col gap-3 max-w-xl">
          {TRIGGER_LABELS.map((label) => {
            ...
            return (
              <div
                key={label}
                className="flex items-center justify-between px-5 py-4 rounded-md border bg-gray-50 border-gray-100 transition-colors duration-150 hover:border-gray-200"
              >
                {/* Label */}
                ...
                <div className="flex items-center gap-3 ml-4 shrink-0">
                  ...
                  {isError && (
                    <span className="text-[11px] text-red-500 bg-red-50 border border-red-200 px-2 py-0.5 rounded-full max-w-[180px] truncate" title={state.message}>
                      ✕ {state.message}
                    </span>
                  )}
                  ...
                  {/* Trigger button */}
                  <button
                    onClick={() => handleTrigger(label)}
                    disabled={isLoading}
                    className="flex items-center gap-2 px-4 py-1.5 rounded text-[12px] text-white cursor-pointer transition-all duration-150 disabled:opacity-60 disabled:cursor-not-allowed"
                    style={{ background: isError ? "#b91c1c" : "#1a3a5c" }}
                    onMouseEnter={(e: React.MouseEvent<HTMLButtonElement>) => {
                      if (!isLoading) e.currentTarget.style.background = isError ? "#991b1b" : "#245080";
                    }}
                    onMouseLeave={(e: React.MouseEvent<HTMLButtonElement>) => {
                      e.currentTarget.style.background = isError ? "#b91c1c" : "#1a3a5c";
                    }}
                  >
                    {isLoading ? (
                      <>
  ...
  return (
    ...
    >
      ...
      <main className="flex flex-col flex-1 overflow-hidden bg-white">
        {selected.type === "commands"  && <CommandsPanel item={selected as CommandsItem} />}
        {selected.type === "compare"   && <ComparePanel />}
        {selected.type === "triggers"  && <TriggersPanel />}
