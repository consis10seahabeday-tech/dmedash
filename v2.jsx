...
function CommandsPanel({ item }) {
  return (
    <>
      <header style={{ borderBottom: "1px solid #1a1a1a", padding: "20px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={{ fontSize: 12, color: "#3a3a3a" }}>{item.icon}</span>
            <h1 style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#c0c0c0", margin: 0 }}>{item.label}</h1>
          </div>
          <p style={{ fontSize: 11, color: "#3a3a3a", marginTop: 3 }}>{item.commands.length} commands available</p>
        </div>
        <div style={{ fontSize: 11, padding: "4px 12px", borderRadius: 4, color: "#2a4a2a", background: "#111811", border: "1px solid #1a2e1a" }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "24px 32px" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, maxWidth: 760 }}>
          {item.commands.map((command, i) => (
            <div
              key={i}
              style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderRadius: 6, background: "#111111", border: "1px solid #1e1e1e", transition: "border-color 0.15s" }}
              onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#2a2a2a"; }}
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#1e1e1e"; }}
            >
              <div style={{ flex: 1, marginRight: 24 }}>
                <div style={{ fontSize: 11, color: "#555555", marginBottom: 5, letterSpacing: "0.04em" }}>{command.label}</div>
                <code style={{ fontSize: 13, color: "#d4d4d4", fontFamily: "monospace", letterSpacing: "0.02em" }}>{command.cmd}</code>
              </div>
      ...
      <header style={{ borderBottom: "1px solid #1a1a1a", padding: "20px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <h1 style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#c0c0c0", margin: 0 }}>Compare</h1>
          </div>
          <p style={{ fontSize: 11, color: "#3a3a3a", marginTop: 3 }}>Preprod vs Production</p>
        </div>
        <div style={{ fontSize: 11, padding: "4px 12px", borderRadius: 4, color: "#2a4a2a", background: "#111811", border: "1px solid #1a2e1a" }}>
          <span style={{ color: "#3a6a3a" }}>● </span>ready
        </div>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            {["Preprod", "Prod"].map((env) => (
              <div key={env} style={{ flex: 1, display: "flex", flexDirection: "column", gap: 8 }}>
                <label style={{ fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", color: "#4a4a4a" }}>{env}</label>
                <textarea
                  rows={15}
                  placeholder={`Paste ${env.toLowerCase()} output here...`}
                  style={{
                    width: "100%", background: "#111111", border: "1px solid #1e1e1e",
                    color: "#c0c0c0", padding: "14px 16px", fontFamily: "monospace",
          ...
          <div>
            <button
              style={{
                ...
                background: "#1a1a1a", color: "#c0c0c0", border: "1px solid #2a2a2a",
    ...
    <>
      <header style={{ borderBottom: "1px solid #1a1a1a", padding: "20px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <h1 style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#c0c0c0", margin: 0 }}>Report</h1>
            <span style={{ fontSize: 11, padding: "3px 10px", borderRadius: 4, color: "#4a4a4a", border: "1px solid #1e1e1e", background: "#111" }}>{formattedDate}</span>
          </div>
          <p style={{ fontSize: 11, color: "#3a3a3a", marginTop: 3 }}>Editable health check report</p>
        </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "24px 32px" }}>
        <textarea
          ...
          style={{
            width: "100%", minHeight: 520, background: "#111111",
            border: "1px solid #1e1e1e", color: "#c0c0c0",
...
export default function DMEDashboard() {
  ...
  const NavItem = ({ item }) => {
    ...
    return (
      ...
      >
        <span style={{ fontSize: 11, opacity: isActive ? 1 : 0.45 }}>{item.icon}</span>
        <span style={{ letterSpacing: "0.02em" }}>{item.label}</span>
        {isActive && item.commands && (
          <span style={{ marginLeft: "auto", fontSize: 11, color: "#3a3a3a" }}>{item.commands.length}</span>
        )}
      </button>
    );
  };
  ...
  return (
    <div style={{ display: "flex", height: "100vh", width: "100%", overflow: "hidden", background: "#0d0d0d", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif" }}>
      {/* Sidebar */}
      <aside style={{ display: "flex", flexDirection: "column", width: 200, height: "100%", flexShrink: 0, borderRight: "1px solid #1e1e1e" }}>
        <div style={{ padding: "20px", borderBottom: "1px solid #1e1e1e" }}>
          <div style={{ fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "#3f3f3f" }}>dashboard</div>
          <div style={{ fontSize: 18, fontWeight: 700, letterSpacing: "0.1em", color: "#e5e5e5", marginTop: 2 }}>DME</div>
        </div>
        ...
        <nav style={{ display: "flex", flexDirection: "column", flex: 1, padding: "12px 10px", overflowY: "auto" }}>
          ...
          <div style={{ margin: "12px 0", borderTop: "1px solid #1a1a1a" }} />
          <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <div style={{ padding: "14px 20px", borderTop: "1px solid #1a1a1a" }}>
          <div style={{ fontSize: 11, color: "#2e2e2e" }}>v1.0.0</div>
        </div>
      </aside>
